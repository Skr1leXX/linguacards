...
⚠️ Backend развёрнут на бесплатном тарифе Render — при первом заходе после простоя сервер "просыпается" до ~30-50 секунд. Это нормально, не баг.

ℹ️ Регистрация по email работает через Resend, но на бесплатном тарифе письма с кодом подтверждения уходят только на почту разработчика — для полноценного теста используй готовый тестовый аккаунт выше.
